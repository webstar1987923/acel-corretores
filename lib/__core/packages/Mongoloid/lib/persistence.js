...
let path = { join(...args) { return args.join('_'); } },
  model = require('./model'),
  async = require('async'),
...
Persistence.prototype.persistCachedDatabase = function (cb) {
  ...
  Object.keys(this.db.indexes).forEach((fieldName) => {
    if (fieldName != '_id') {   // The special _id index is managed by datastore.js, the others need to be persisted
      toPersist += `${self.afterSerialization(model.serialize({ $$indexCreated: { fieldName, unique: self.db.indexes[fieldName].unique, sparse: self.db.indexes[fieldName].sparse } }))}\n`;
    }
  });
...
Persistence.prototype.persistNewState = function (newDocs, cb) {
  ...
  storage.appendFile(self.filename, toPersist, 'utf8', err => callback(err));
};
...
Persistence.prototype.loadDatabase = function (cb) {
  ...
  async.waterfall([
    function (cb) {
      Persistence.ensureDirectoryExists(self.filename, (err) => {
